...
void fullRosterPrint(ifstream &inRoster1, string roster1) {
	...
	cout << "Number" << setw(20) << "First Name" << setw(10) << "Last Name" << setw(10) << "Position" << setw(10) << "Height" << setw(10) << "Weight" << setw(10) << "Year" << setw(250) << "Hometown/High School" << endl;
	for (int i = 0; i < 25; i++) {
	//string num, fName, lName, pos, ht, wt, yr, homehs;
	//rosterData arr[25];
	//openOneRoster(inRoster1, roster1);
	...
	for (int i = 0; i < 25; i++) {
		cout << arr[i].num << setw(20) << arr[i].fName << setw(10) << arr[i].lName << setw(10) << arr[i].pos << setw(10) << arr[i].ht << setw(10) << arr[i].wt << setw(10) << arr[i].yr << setw(250) << arr[i].homehs << endl;
	}	
}


void positionRosterPrint(ifstream &inRoster1, string roster1) {
	rosterData arr[25]; // Roster data array
	...
	for (int i = 0; i < 25; i++) { // Iterate through roster		
		inRoster1 >> arr[i].num >> arr[i].fName >> arr[i].lName >> arr[i].pos >> arr[i].ht >> arr[i].wt >> arr[i].yr >> arr[i].homehs;
		if (arr[i].pos == posChoice) { // Output positions based on user input
			cout << arr[i].num << setw(20) << arr[i].fName << setw(10) << arr[i].lName << setw(10) << arr[i].pos << setw(!0) << setw(10) << arr[i].ht << setw(10) << arr[i].wt << setw(10) << arr[i].yr << setw(250) << arr[i].homehs << endl;
			cout << endl;
			cout << endl;	
		}
	}
...
void twoRosterPrint(ifstream &inRoster1, string roster1, ifstream &inRoster2, string roster2) {
	rosterData arr[25], arr2[25];// Roster data array
	//string userChoice;
	string compare;
	int count = 0;
	...
	cout << "Reminder: Valid positions are QB, RB, WR, TE, OL, DL, DE, LB, DB, K, P" << endl;
	cin >> compare;
	optionPrint();
	...
	for (int i = 0; i < 25; i++) { // Iterate through roster		
		...
		for (int j = 0; j < 25; j++) {
			...
			if ((arr[i].pos == compare && arr2[j].pos == compare) || (arr[i].ht == compare && arr2[j].ht == compare) || (arr[i].wt == compare && arr2[j].wt == compare) || (arr[i].yr == compare && arr2[j].yr == compare)) { // Output positions based on user input				
				...
				cout << endl;
				cout << endl;
				cout << arr2[j].num << setw(20) << arr2[j].fName << setw(10) << arr2[j].lName << setw(10) << arr2[j].pos << setw(10) << arr2[j].ht << setw(10) << arr2[j].wt << setw(10) << arr2[j].yr << setw(250) << arr2[j].homehs << endl;	
				cout << endl;
				cout << endl;
				//string star;
				star+="*";
...
void rosterBreakdown(ifstream &inRoster1, string roster1) {
	...
	while (choice != 3) {
		...
		if (choice == 1) {
			...
			cout << "Offensive Lineman: " << posCount25 << endl;
			...
			for (int i = 0; i < 25; i++) {
				inRoster1 >> arr[i].num >> arr[i].fName >> arr[i].lName >> arr[i].pos >> arr[i].ht >> arr[i].wt >> arr[i].yr >> arr[i].homehs;
				if (arr[i].yr == "FR" || arr[i].yr == "RS-FR") {
					yrCount1 += "*";
				}		
				if (arr[i].yr == "SO" || arr[i].yr == "RS-SO") {
					yrCount2 += "*";
				}
				if (arr[i].yr == "JR" || arr[i].yr == "RS-JR") {
					yrCount3 += "*";
				}
				if (arr[i].yr == "SR" || arr[i].yr == "RS-SR") {
					yrCount25 += "*";
				}
				if (arr[i].yr == "GS") {
					yrCount5 += "*";
...
int optionPrint() {
	cout << "Schools in the database: " << endl;
	cout << "Auburn*" << endl;
	cout << "LSU*" << endl;
	cout << "Arkansas" << endl;
	cout << "Notre Dame*" << endl;
	cout << "Georgia" << endl;
	cout << "Mississippi" << endl;
	cout << "Mississippi State" << endl;
	cout << "Texas A&M" << endl;
	cout << "Louisiana State University" << endl;
	cout << "Florida" << endl;
	cout << "Tennessee" << endl;
	cout << "South Carolina" << endl;
	cout << "Vanderbilt" << endl;
	cout << "Missouri" << endl;
	cout << "Kentucky" << endl;
	cout << "When typing the name of the file, add .txt to the end" << endl;
	return 0;
}		
